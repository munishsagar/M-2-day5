...
const giveMeRandom = [1, 4, 6, 7, 2, 3, 5];
let sum = 0;
const checkArray = function (num) {
  if (num > 5) {
    sum += num;
    return sum;
  }
...
for (let i = 0; i < giveMeRandom.length; i++) {
  console.log(checkArray(giveMeRandom[i]));
}
...
const addToShoppingCart = function (obj) {
  ...
  return console.log(
    `number of items in shopping cart are :${shoppingCart1.length}`
  );
};
addToShoppingCart({ name: "jack jones", price: 20, quantity: 4, id: 1238 });
/* EXTRA 4
 In your eCommerce you have an array of objects called shoppingCart. Each one of these objects has a price, a name, an id and the quantity to be shipped.
...
//console.log(shoppingCart3.length - 1);
const latestShoppingCart = function (cart) {
  const lastItem = cart[cart.length - 1];
  console.log(lastItem);
};

latestShoppingCart(shoppingCart3);
...
console.log(loopUntil(Math.trunc(Math.random() * 10) + 1));
...
console.log(average(["sagar", 20, 5, "3", "munish", 8, 9]));
...
const longest = function (str) {
  ...
  for (let i = 0; i < str.length; i++) {
    const newest = str[i].length;
    ar.push(newest);
  }
  return ar;
};

const output = longest([
  "munishsagar",
  "student",
  "in",
  "epicode",
...
console.log(longestt(["strive", "school", "developer", "student"]));
...
const filter = function (filter) {
  if (filter.includes("spam")) {
    return true;
  } else {
    return false;
  }
};
console.log(filter("spam"));
console.log(filter("scam"));
/* EXTRA 10
...
const searchJobs = (jobs, position, location) => {
  //return jobs.filter(job => job.title.includes(position) && job.location.includes(location))
  let result = [];
...
console.log(searchJobs(jobs, "Front End Developer", "nz"));
